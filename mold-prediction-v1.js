...
function displayPredictionResults(prediction) {
    ...
    prediction.moldOrders.forEach(order => {
        ...
        const endDate = new Date(order.orderDate);
...
function drawMoldOrderChart(prediction) {
    ...
    const moldData = prediction.moldOrders.map(order => {
        return {
            ...
            lifeEndMonth: Math.ceil(prediction.moldLifeShots * prediction.cavityCount / prediction.monthlyPlan[order.monthIndex])
        };
    });
    ...
    const datasets = [
        {
            ...
            data: moldData.map(data => ({
                x: [new Date(data.orderDate.getTime() + data.leadTime * 24 * 60 * 60 * 1000).getTime(), 
                    new Date(data.orderDate.getTime() + data.leadTime * 24 * 60 * 60 * 1000 + data.lifeEndMonth * 30 * 24 * 60 * 60 * 1000).getTime()],
                y: data.moldNumber
            })),
        ...
        },
        {
            label: '生産終了時期',
            data: moldData.map(data => ({
                x: new Date(data.orderDate.getTime() + data.lifeEndMonth * 30 * 24 * 60 * 60 * 1000).getTime(),
                y: data.moldNumber
            })),
        ...
        options: {
            ...
            scales: {
                x: {
                    ...
                    min: today.getTime(),
                    max: Math.max(...moldData.map(data => new Date(data.orderDate.getTime() + data.lifeEndMonth * 30 * 24 * 60 * 60 * 1000).getTime())),
                    grid: {
