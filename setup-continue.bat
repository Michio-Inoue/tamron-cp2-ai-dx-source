...
echo AIzaSyC39YRwttNDsVj6ZSWLhq8ljbiGyW9YcZo | gcloud secrets create gemini-api-key --data-file=- --replication-policy="automatic" --project=singular-server-480006-s8 2>nul
if %errorlevel% neq 0 (
    echo シークレットが既に存在する可能性があります。更新を試みます...
    echo AIzaSyC39YRwttNDsVj6ZSWLhq8ljbiGyW9YcZo | gcloud secrets versions add gemini-api-key --data-file=- --project=singular-server-480006-s8
    if %errorlevel% neq 0 (
